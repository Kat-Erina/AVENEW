...
const CustomSelect = ({ options, value, onChange, placeholder }) => {
  ...
  return (
    <div className="relative w-full">
      ...
      >
        <span className="text-sm tracking-widest uppercase text-[16px]"> {!value ? placeholder : value}</span>
...
<div className="flex items-center border-b border-white/30 h-14 px-4 w-full">
  <input
    ...
    placeholder={t('name')}
    className="flex-1 bg-transparent text-yellowish text-[16px] tracking-widest uppercase placeholder:text-yellowish outline-none"
  />
...
{errors.tel && <p className="text-red-400 text-xs px-4 py-1">{errors.tel}</p>}
      ...
      <button onClick={()=>{handleSubmit()}} className="h-14 border border-white/30 text-yellowish text-[16px] tracking-widest uppercase hover:bg-white/10 transition-colors">
        {t('requestACall')}
      </button>
